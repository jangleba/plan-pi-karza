...
export function choreograph(scenario: SimScenario): SimActor[] {
  ...
  const bestZone = [...scenario.zones].sort(
    (z1, z2) => z2.quality - z1.quality,
  )[0];
  const keyReaction =
    scenario.reactions.find(
      (r) => r.id === (bestZone?.reaction ?? scenario.defaultReaction),
  ...
  const supportOptions = selfStart
    ? [...scenario.actors]
        ...
        .sort(
          (p, q) =>
            dist(actorAt(p.path, 0), selfStart) -
            dist(actorAt(q.path, 0), selfStart),
        )
    : [];
  ...
  const widthPlayer = [...scenario.actors]
    ...
    .sort(
      (a, b) =>
        Math.abs(actorAt(b.path, 0).x - 50) -
        Math.abs(actorAt(a.path, 0).x - 50),
    )[0];
