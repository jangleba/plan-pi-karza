...
function invalid(
  ...
  return {
    ...
    markedBy: "user",
    derived: {},
    basis: { method: message, items: [{ label: "FPS filmu", value: `${fps || "—"}` }], coachVerifiedFrames: false },
  };
}
...
export function computeFrameResult(params: {
  ...
  if (testId === "cmj") {
    ...
    if (takeoff == null) return invalid(testId, "jump", fps, markers, manual, "Brak klatki oderwania.");
    if (landing == null) return invalid(testId, "jump", fps, markers, manual, "Brak klatki lądowania.");
    if (landing <= takeoff)
      return invalid(testId, "jump", fps, markers, manual, "Klatka lądowania musi być po klatce oderwania.");
    const frameCount = landing - takeoff;
    const flightTime = frameCount / fps;
    const heightM = (G * flightTime * flightTime) / 8;
    ...
    const basis: CalculationBasis = {
      ...
      items: [
        ...
        { label: "Czas lotu", value: `${round(flightTime, 3)} s` },
        { label: "Wzór", value: "h = 9.81 × t² / 8" },
        { label: "Wysokość skoku", value: `${heightCm} cm` },
        { label: "Oznaczone przez", value: markedByLabel(markedBy) },
      ],
    };
    return {
      testId, category: "jump", fps, markers, manual,
      status: statusFor(estimated, markedBy),
      error: null,
      mainResultValue: heightCm, mainResultUnit: "cm",
      method: "Flight Time Method", markedBy, derived, basis,
    };
  }
  ...
  if (testId === "broad_jump") {
    ...
    if (distanceCm == null || distanceCm <= 0)
      return invalid(testId, "jump", fps, markers, manual, "Odległość wymaga ręcznego pomiaru albo kalibracji skali.");
    const derived: FrameDerived = { distanceCm: round(distanceCm, 0), distanceM: round(distanceCm / 100, 2) };
    const basis: CalculationBasis = {
    ...
    return {
      ...
      error: null,
      mainResultValue: round(distanceCm, 0), mainResultUnit: "cm",
      method: "Manual Measurement", markedBy, derived, basis,
    };
  }
  ...
  if (testId === "pogo_jumps") {
    ...
    if (first == null) return invalid(testId, "jump", fps, markers, manual, "Brak klatki pierwszego kontaktu.");
    if (last == null) return invalid(testId, "jump", fps, markers, manual, "Brak klatki ostatniego kontaktu.");
    if (last <= first) return invalid(testId, "jump", fps, markers, manual, "Ostatni kontakt musi być po pierwszym.");
    ...
    return {
      ...
      status: statusFor(estimated, markedBy),
      error: null,
      mainResultValue: round(rhythm, 2), mainResultUnit: "/s",
      method: "Frame-Based Rhythm", markedBy, derived, basis,
    };
  }
  ...
  if (testId === "sprint_20m" || testId === "sprint_30m") {
    ...
    if (start == null) return invalid(testId, "sprint", fps, markers, manual, "Brak klatki startu.");
    if (finish == null) return invalid(testId, "sprint", fps, markers, manual, "Brak klatki mety.");
    if (finish <= start) return invalid(testId, "sprint", fps, markers, manual, "Klatka mety musi być po klatce startu.");
    ...
    return {
      testId, category: "sprint", fps, markers, manual,
      status: statusFor(estimated, markedBy),
      error: null,
      mainResultValue: round(sprintTime, 2), mainResultUnit: "s",
      method: "Frame-Based Timing", markedBy, derived, basis,
    };
  }
  ...
  if (testId === "five_ten_five" || testId === "sprint_to_stop") {
    ...
    if (brakingStart == null) return invalid(testId, "cod", fps, markers, manual, "Brak klatki początku hamowania.");
    if (stop == null) return invalid(testId, "cod", fps, markers, manual, "Brak klatki zatrzymania.");
    if (stop <= brakingStart) return invalid(testId, "cod", fps, markers, manual, "Zatrzymanie musi być po początku hamowania.");
    const brakingTime = (stop - brakingStart) / fps;
    const totalTime =
      exit != null && entry != null && exit > entry ? (exit - entry) / fps : null;
    const derived: FrameDerived = {
    ...
    if (totalTime != null) items.push({ label: "Czas całkowity", value: `${round(totalTime, 3)} s` });
    items.push(
      ...
      { label: "Liczba kroków hamowania", value: manual.braking_steps != null ? `${manual.braking_steps}` : "—" },
      { label: "Oznaczone przez", value: markedByLabel(markedBy) },
    );
    const basis: CalculationBasis = {
    ...
    return {
      testId, category: "cod", fps, markers, manual,
      status: statusFor(estimated, markedBy),
      error: null,
      mainResultValue: round(brakingTime, 2), mainResultUnit: "s",
      method: "Frame Braking Analysis", markedBy, derived, basis,
    };
  }

  return invalid(testId, test.category, fps, markers, manual, "Ten test nie obsługuje analizy klatkowej.");
}
