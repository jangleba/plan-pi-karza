...
export async function resolveVideoBlob(opts: {
  ...
  // 1. Jeśli upload do chmury się udał, analizuj świeżo pobrany Blob z chmury.
  // To eliminuje przypadki, w których File z inputa po zmianie trasy staje się
  // nieczytelny dla elementu <video> w Safari/iOS.
  if (videoUrl && uploaded && !videoUrl.startsWith("placeholder://")) {
    try {
      ...
      if (!file) throw e;
      // Fallback do lokalnego File, jeśli chmura chwilowo zawiedzie.
    }
  }

  // 2. Lokalny plik z uploadu — fallback i tryb offline.
  if (file) {
