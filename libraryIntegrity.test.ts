...
describe("BallWise IQ — integralność biblioteki mikrosymulacji", () => {
  it("każdy scenariusz jest kompletny, osiągalny i deterministyczny", () => {
    ...
    for (const scenario of SIM_SCENARIOS) {
      ...
      for (const reaction of scenario.reactions) {
        ...
        expect(
          action,
          `${scenario.id}: brak wyniku dla ${reaction.id}`,
        ).toBeDefined();
        const alternative = scenario.alternatives[reaction.id];
        expect(
          alternative,
          `${scenario.id}: brak alternatywy dla ${reaction.id}`,
        ).toBeDefined();
        expect(
          scenario.actions.some(
            (candidate) => candidate.id === alternative?.actionId,
          ),
  ...
  it("ma odrębne scenariusze dla każdej grupy i jeden zaawansowany standard bramkarza", () => {
    expect(
      SIM_SCENARIOS.filter((scenario) =>
        scenario.positions.includes("goalkeeper"),
      ),
    ).toHaveLength(9);
    for (const level of [
      "beginner",
    ...
    expect(
      SIM_SCENARIOS.some((scenario) => scenario.positions.includes("defender")),
    ).toBe(true);
    expect(
      SIM_SCENARIOS.some((scenario) =>
        scenario.positions.includes("midfielder"),
      ),
    ).toBe(true);
    expect(
      SIM_SCENARIOS.some((scenario) => scenario.positions.includes("forward")),
    ).toBe(true);
  });
});
