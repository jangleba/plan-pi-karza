-- BallWise: least-privilege defaults for future public-schema objects and
-- complete removal of locally collected health-history fields after consent
-- withdrawal. Apply after 20260916210000_canonical_consent_and_rls.sql.

begin;

-- Supabase's database roles can otherwise inherit broad privileges on objects
-- created later. Every future migration must grant only the access it needs.
...
-- This is an internal primitive called by record_consent(). The browser must
-- not be able to supply its own legal version or text snapshot.
