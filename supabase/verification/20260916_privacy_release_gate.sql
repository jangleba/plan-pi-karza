...
select
  ...
  (
    ...
    from public.athlete_profiles ap
    where (
      ap.fuel_allergy_status <> 'unconfirmed'
      or cardinality(ap.food_allergies) > 0
      or cardinality(ap.food_intolerances) > 0
    )
    and public.latest_consent_is_accepted(ap.user_id, 'health_data') is not true
...
-- Every user-data table must show rls_enabled=true.
