...
import { isScheduleStepComplete } from "@/lib/onboardingValidation";
              ...
              {seasonPhase !== null &&
                ...
                seasonValidation.status === "incomplete" && (
                  <p className="rounded-xl border border-border bg-muted/50 p-3 text-xs text-muted-foreground">
                    Datę najbliższego meczu możesz podać w kolejnym kroku albo
                    uzupełnić później w profilu.
                  </p>
                )}
              {seasonPhaseOverride && (
        ...
        {step === 4 && (
          <div className="space-y-8">
            ...
            <div className="space-y-7 rounded-2xl border border-border bg-card p-5">
              ...
              {/* Data meczu */}
              <div className="space-y-2.5">
                <Label>Data najbliższego meczu (opcjonalnie)</Label>
                <Popover>
                  <PopoverTrigger asChild>
                    <button
                      type="button"
                      className="flex w-full items-center gap-3 rounded-xl border border-border bg-background px-4 py-3 text-left text-sm transition-colors"
                    >
                ...
                <div className="flex items-start justify-between gap-3">
                  <p className="text-xs text-muted-foreground">
                    Jeśli jeszcze nie znasz terminu, plan powstanie bez niego.
                    Datę możesz dodać później.
                  </p>
                  {matchDate && (
                    <button
                      type="button"
            ...
            <div className="space-y-2.5">
              ...
              <label className="flex items-start gap-3 rounded-xl border border-border bg-card p-3.5">
                <Checkbox
                  checked={hasSprintSpace}
                  onCheckedChange={(c) => setHasSprintSpace(c === true)}
                  className="mt-0.5"
                />
                <span className="text-sm">
                  Mam bezpieczne miejsce do sprintu
                  <span className="mt-0.5 block text-xs text-muted-foreground">
                    Minimum 15–30 metrów prostej, równej nawierzchni.
                  </span>
                </span>
              </label>
              <label className="flex items-start gap-3 rounded-xl border border-border bg-card p-3.5">
